...
def m3u_to_json(filename):
    ...
    for line in lines:
        ...
        if line.startswith("#EXTINF:"):
            ...
            group_name = group_title_part.split('group-title="')[1].split('"')[0] if 'group-title="' in group_title_part else "未分类"

            # 查找现有的频道是否已存在
            existing_channel = next((ch for ch in current_channel_list if ch["name"] == channel_name), None)
            ...
            if current_channel_list:
                # 查找当前频道
                existing_channel = next((ch for ch in current_channel_list if ch["name"] == channel_name), None)
                if existing_channel and line not in existing_channel["urls"]:
                    existing_channel["urls"].append(line)
